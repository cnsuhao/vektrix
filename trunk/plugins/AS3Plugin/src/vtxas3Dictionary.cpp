...
namespace vtx { namespace as3 {
	...
	//-----------------------------------------------------------------------
	Atom FASTCALL Dictionary::getKeyFromObject(Atom key) const
	{
	...
	//-----------------------------------------------------------------------
	Atom Dictionary::getAtomProperty(Atom key) const
	{
	...
	//-----------------------------------------------------------------------
	bool Dictionary::hasAtomProperty(Atom key) const
	{
		if (AvmCore::isObject(key))
		{
	...
	//-----------------------------------------------------------------------
	bool Dictionary::deleteAtomProperty(Atom key)
	{
		if (AvmCore::isObject(key))
		{
	...
	//-----------------------------------------------------------------------
	void Dictionary::setAtomProperty(Atom key, Atom value)
	{
		if (AvmCore::isObject(key))
		{
	...
	//-----------------------------------------------------------------------
	Atom Dictionary::nextName(int index)
	{
		AvmAssert(index > 0);
		...
		{
			MMgc::GCWeakRef* ref = (MMgc::GCWeakRef*)AvmCore::atomToGenericObject(k);
			union {
				MMgc::GCObject* key_o;
				ScriptObject* key;
			};
	...
	//-----------------------------------------------------------------------
	Atom Dictionary::nextValue(int index)
	{
		AvmAssert(index > 0);
	...
	//-----------------------------------------------------------------------
	int Dictionary::nextNameIndex(int index)
	{
		AvmAssert(index >= 0);
